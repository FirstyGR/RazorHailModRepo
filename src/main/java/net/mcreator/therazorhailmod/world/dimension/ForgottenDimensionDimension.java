...
package net.mcreator.therazorhailmod.world.dimension;
...
public class ForgottenDimensionDimension extends TherazorhailModModElements.ModElement {
	@ObjectHolder("therazorhail_mod:forgotten_dimension_portal")
	public static final CustomPortalBlock portal = null;
	public ForgottenDimensionDimension(TherazorhailModModElements instance) {
		super(instance, 65);
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("therazorhail_mod:forgotten_dimension"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("forgotten_dimension_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("forgotten_dimension_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("forgotten_dimension_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new ForgottenDimensionItem().setRegistryName("forgotten_dimension"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.addParticle(DementiaParticlesParticle.particle, px, py, pz, vx, vy, vz);
			}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("therazorhail_mod:forgotten_dimension"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("therazorhail_mod:forgotten_dimension")));
				} else {
					entity.func_242279_ag();
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? Blocks.COAL_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.COAL_BLOCK.getDefaultState(), 3);
					}
				}
			}
